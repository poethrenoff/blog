...
class module_search extends module
{
	...
	*/
	...
	{
		$records_per_page = max( intval( $this -> get_param( 'records_per_page' ) ), 1 );
		...
		if ( $search_tag !== '' || $search_text !== '' )
		{
			...
			$search_count = db::select_row( $search_query, $filter_binds ); $search_count = $search_count['_search_count'];
			
			$pages = paginator::construct( $search_count, array( 'by_page' => $records_per_page ) );
			...
			foreach( $search_list as $result_index => $result_item )
			{
				...
				$search_list[$result_index]['news_content'] =
					$this -> prepare_search_result( strip_tags( $result_item['news_content'] ), $search_words );
				...
				$search_list[$result_index]['news_url'] =
					url_for( array( 'controller' => '', 'action' => 'post', 'id' => $result_item['news_id'] ) );
			}
		...
		$this -> assign_tag_cloud( $search_tag );
		
		$this -> content = $this -> view -> fetch( 'module/search/search.tpl' );
	}
	
	/**
	* Преобразование результатов поиска
	*/
	protected function prepare_search_result( $result_text, $search_words, $search_limit = 200 )
	{
		$result_text = strip_tags( $result_text );
		
		$result_text_length = mb_strlen( $result_text );
		$result_pos_min = $result_text_length; $result_pos_max = 0;
		...
		foreach ( $search_words as $search_word )
		{
			...
			{
				if ( $result_left_pos < $result_pos_min ) $result_pos_min = $result_left_pos;
				if ( $result_right_pos > $result_pos_max ) $result_pos_max = $result_right_pos + mb_strlen( $search_word );
	...
	* Облако тегов
	*/
	protected function assign_tag_cloud( $search_tag = '' )
	{
		$tag_cloud_count = max( intval( $this -> get_param( 'tag_cloud_count' ) ), 1 );
		
		$min_font_size = max( intval( $this -> get_param( 'min_font_size' ) ), 1 );
		$max_font_size = max( intval( $this -> get_param( 'max_font_size' ) ), 1 );
